defmodule GwordleWeb.HomeLive.Index do
  ...
  def handle_event("previous_wordle", _data, socket) do
    move_day(-1, socket)
  end

  defp move_day(difference, socket) do
    if socket.assigns.selected_wordle_day + difference <= socket.assigns.wordle_day and
         socket.assigns.selected_wordle_day + difference > 0 do
      submission =
        Submissions.get_submission_by_user_id_wordle_day(
          socket.assigns.current_user.id,
          socket.assigns.selected_wordle_day + difference
        )

      socket =
        socket
        |> assign_submission(submission)
        |> assign(:selected_wordle_day, socket.assigns.selected_wordle_day + difference)
