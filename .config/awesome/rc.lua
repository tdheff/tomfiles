...
local themer = require("src.themes.themer")
...
function Hover_signal(widget, bg, fg)
    ...
    local mouse_leave = function()
        if bg then widget.bg = old_bg end
        if fg then widget.fg = old_fg end
        if old_wibox then
...
awful.spawn.with_shell("xrandr --output DP-0 --mode 3440x1440 --rate 180.00")
